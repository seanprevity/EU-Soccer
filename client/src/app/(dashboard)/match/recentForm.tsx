...
import { renderForm, StatBar } from "@/lib/uiUtils";
import {
  getLogoFile,
  getResultColors,
  getMatchResult,
...
const RecentForm = ({
  ...
  const { data: last5Home, isLoading: isHomeMatchesLoading } =
    useGetLast5MatchesQuery({
      team: homeTeam,
    });
  const { data: last5Away, isLoading: isAwayMatchesLoading } =
    useGetLast5MatchesQuery({
      team: awayTeam,
    });
  const [expandedMatchId, setExpandedMatchId] = useState<number[]>([]);

  if (!last5Away || !last5Home) return <div>No Recent Matches.</div>;
    ...
    <div className="w-full pt-2">
      ...
      <div className="flex flex-col gap-1 md:flex-row w-full items-start justify-center">
        ...
        <div className="w-full md:w-1/2 px-2 self-stretch">
          ...
          {isHomeMatchesLoading ? (
            <>
              <div>Loading ${homeTeam} Recent Matches...</div>
            </>
          ) : (
            <ul className="max-w-xl mx-auto mb-6 space-y-3 sm:space-y-4">
              {last5Home?.map((m) => {
                const result = getMatchResult(m, homeTeam);
                  ...
                  >
                    <button
                      ...
                      className={`relative w-full flex items-center ${
                        colors.gradient
                      } p-2 sm:p-3 hover:bg-gray-100 dark:hover:bg-gray-700 hover:rounded-md cursor-pointer transition text-left gap-3
                ${
        ...
        <div className="w-full md:w-1/2 px-2 self-stretch">
          ...
          {isAwayMatchesLoading ? (
            <>
              <div>Loading ${awayTeam} Recent Matches...</div>
            </>
          ) : (
            <ul className="max-w-xl mx-auto mb-6 space-y-3 sm:space-y-4">
              {last5Away?.map((m) => {
                const result = getMatchResult(m, awayTeam);
                const colors = getResultColors(result);
                return (
                  <li
                    key={m.id}
